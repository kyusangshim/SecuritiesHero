...
@Entity
@Table(name = "dart_public_offering_fund_usage")
@Data
@NoArgsConstructor
@AllArgsConstructor
public class DartPublicOfferingFundUsage {
    ...
    @Column(name = "on_dclrt_cptal_use_plan", columnDefinition = "TEXT")
    private String onDclrtCptalUsePlan; // 신고서상 자금사용 계획

    @Column(name = "real_cptal_use_sttus", columnDefinition = "TEXT")
    private String realCptalUseSttus; // 실제 자금사용 현황

    // --- 2018년 1월 19일 이후 필드 ---
    @Column(name = "rs_cptal_use_plan_useprps", columnDefinition = "TEXT")
    private String rsCptalUsePlanUseprps; // 증권신고서 등의 자금사용 계획(사용용도)

    @Column(name = "rs_cptal_use_plan_picure_amount")
    private Long rsCptalUsePlanPicureAmount; // 증권신고서 등의 자금사용 계획(조달금액)
